...
%SE = strel('line',5,30);  %len es llargada i deg els graus 
...
%             if video==1
%                 subplot(1,2,1); imshow(uint8(grayscale));
%                 title('Sequence')
%                 subplot(1,2,2); imshow(logical((detection)));
%                 title('Sequence Highway segmentation, Closing with alpha=6 and Diamond as SE ')
