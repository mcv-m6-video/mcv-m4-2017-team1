...
tracks = initializeTracks_MS(); % Create an empty array of tracks.
...
for seq=1
    ...
    for i = iniFrame(seq)+25:endFrame(seq)
        ...
        tracks=predictNewLocationsOfTracks_MS(tracks,image_);
        [assignments, unassignedTracks, unassignedDetections] = ...
            detectionToTrackAssignment_MS(tracks,bboxes);
        
        tracks=updateAssignedTracks_MS(assignments,bboxes,tracks);
        tracks=updateUnassignedTracks_MS(tracks,unassignedTracks);
        tracks=deleteLostTracks_MS(tracks);
        [nextId,tracks]=createNewTracks_MS(tracks,unassignedDetections,bboxes,nextId,image_);
        ...
        [speed_limit_pictures,speed_limit_id,speed_limit_labels]= displayTrackingResults_MS(image_,detection_,tracks,velocity,speed_limit_pictures,speed_limit_id,speed_limit_labels);
        if video==1
            F(ind) = getframe(gcf);
            writeVideo(v,F(ind));
            ind=ind+1;
